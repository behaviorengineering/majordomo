...
final Map PIPELINE_CONFIG = [
    ...
    credentials: [
        artifactoryCredentialsId:    'your-artifactory-token-credential-id',
        ...
        stage('Pipeline Guard') {
            ...
            stages {
                stage('Safe Checkout') {
                    ...
                    // alpine is pulled via Artifactory pull-through cache (no Docker Hub access).
...
Copy the template and configure it:
  ...
  git add .majordomo-config.groovy && git commit -m 'Add Majordomo config'"""
                ...
                stage('PR Review') {
                    ...
                    // This pipeline is designed for internal Westpac infrastructure where these
                    // endpoints are stable. Non-Westpac deployments should override HTTP_PROXY,
...
def ensureImages(Map cfg, Map deps) {
    withCredentials([
        ...
        usernamePassword(
            credentialsId: cfg.credentials.artifactoryCredentialsId,
