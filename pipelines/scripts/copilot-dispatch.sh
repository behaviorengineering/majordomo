...
#     --summary          Synthesises all reports into summary.md (iterative, driven by summary-loop.py)
#     --score            Scores an existing summary.md against the rubric; writes score.md
#     --technical        Produces a technical deep-dive report from the full diff (iterative, driven by tech-review-loop.py)
...
#     --technical-deep   Focused second pass on one file cited in tech-review.md (driven by tech-review-deep.py)
#
# Environment:
#   GITHUB_TOKEN              GitHub token with Copilot access (required)
#   COPILOT_PIPELINE          Orchestrator agent name to invoke (default: pr-review)
...
#   COPILOT_CONCURRENCY       Max parallel batches per wave — set in Groovy stage (default: 6)
#   COPILOT_MODEL             Model for file-review batches, finalize, and prose modes
#   COPILOT_SUMMARY_MODEL     Model for summary mode (falls back to COPILOT_MODEL)
#   COPILOT_TECHNICAL_MODEL   Model for technical mode (falls back to COPILOT_MODEL)
#   COPILOT_DEEP_TECHNICAL_MODEL Model for technical-deep mode (falls back to COPILOT_TECHNICAL_MODEL, then COPILOT_MODEL)
#   COPILOT_SCORE_MODEL       Model for score/tech-score modes (default: auto; must not be empty if set)
...
if [ -z "${GITHUB_TOKEN:-}" ]; then
    log_error "GITHUB_TOKEN is required for Copilot CLI authentication"
    exit 1
fi
...
# technical-deep: manifest is already written by tech-review-deep.py into the batch dir
...
# ---------------------------------------------------------------------------
# Technical-deep mode: SKILL + staging already set up by tech-review-deep.py.
...
if [ "${MODE}" = "technical-deep" ]; then
    # Technical-deep mode: batch dir was fully prepared by tech-review-deep.py.
    ...
    if [ "${MODE}" = "summary" ] || [ "${MODE}" = "technical" ]; then
    # Summary mode caps each diff to limit total context — see build-all-diffs.py.
    ...
    [ "${MODE}" = "technical" ] && _cap_arg="--cap 150"
        python3 "${SCRIPT_DIR}/build-all-diffs.py" \
            "${STAGING_DIR}/${SKILL}/manifest.json" \
...
case "${MODE}" in
    finalize)
        ...
        EXTRA_MODEL="${COPILOT_MODEL:-gpt-5-mini}"
        ;;
    summary)
        ...
        EXTRA_DIRS=("--add-dir=$(pwd)")
        EXTRA_MODEL="${COPILOT_SUMMARY_MODEL:-${COPILOT_MODEL:-claude-sonnet-4.5}}"
        ;;
    score)
        PROMPT_OUTPUT="$(dirname "${OUTPUT_DIR}")"
        LOG_LABEL="Score: ${PIPELINE} / skill:${SKILL} / iter:${SUMMARY_ITER:-1}"
        SESSION_FILE="${LOGS_DIR}/score_iter_${SUMMARY_ITER:-1}_session.md"
        # No --add-dir: exposing the pipeline output dir causes context bloat (85+ batch files).
        # The scorer finds summary.md via the output path supplied in the prompt.
        _score_model="${COPILOT_SCORE_MODEL:-auto}"
        EXTRA_MODEL="${_score_model:?COPILOT_SCORE_MODEL must not be empty}"
        ;;
    technical)
        PROMPT_OUTPUT="$(dirname "${OUTPUT_DIR}")"
        ...
        EXTRA_DIRS=("--add-dir=$(pwd)")
        EXTRA_MODEL="${COPILOT_TECHNICAL_MODEL:-${COPILOT_MODEL:-claude-sonnet-4.5}}"
        ;;
    technical-deep)
        ...
        EXTRA_DIRS=("--add-dir=$(pwd)")
        EXTRA_MODEL="${COPILOT_DEEP_TECHNICAL_MODEL:-${COPILOT_TECHNICAL_MODEL:-${COPILOT_MODEL:-claude-sonnet-4.6}}}"
        ;;
    tech-score)
        PROMPT_OUTPUT="$(dirname "${OUTPUT_DIR}")"
...
printf '{"timestamp":"%s","pr":"%s","skill":"%s","mode":"%s","batch":"%s","model":"%s","duration_seconds":%d,"exit_code":%d}\n' \
    ...
    "${RUN_DURATION}" "${COPILOT_EXIT}" \
    >> "${OUTPUT_DIR}/metrics.jsonl"

if [ "${COPILOT_EXIT}" -ne 0 ]; then
    log_error "copilot exited with code ${COPILOT_EXIT}"
    exit "${COPILOT_EXIT}"
fi
