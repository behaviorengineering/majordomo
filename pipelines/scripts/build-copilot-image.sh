...
# Build — using BuildKit secrets so package registry credentials are never baked into image layers
# Credentials are injected via Jenkins withCredentials (REGISTRY_USER, REGISTRY_TOKEN env vars)
