...
func TestLLMUsageSummaryFormatTwoTasks(t *testing.T) {
	t.Parallel()
	c := llmusage.New()
	c.Add("typology_slice_catalog", 100, 20, 120)
	c.Add("typology_slice_meaning", 200, 50, 250)
	text := llmusage.Format(c.Snapshot())
	if !strings.Contains(text, "total=370") {
		t.Fatalf("want grand total 370 in %q", text)
	}
	if !strings.Contains(text, "typology_slice_catalog:") || !strings.Contains(text, "typology_slice_meaning:") {
