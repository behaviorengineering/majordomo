...
func listOpenPRs(ctx context.Context, cfg config.RepoConfig, token string) ([]openPR, error) {
	...
	switch scm {
	case "github":
		return listGitHubPRs(ctx, cfg, token)
	case "gitlab":
		return listGitLabMRs(ctx, cfg, token)
	default:
		return nil, fmt.Errorf("scm %q not supported in poll (github|gitlab)", scm)
	}
}

func listGitHubPRs(ctx context.Context, cfg config.RepoConfig, token string) ([]openPR, error) {
...
func listGitLabMRs(ctx context.Context, cfg config.RepoConfig, token string) ([]openPR, error) {
	...
	page := 1
	for {
		req, err := newGitLabMRListRequest(ctx, api, project, page)
		if err != nil {
			return nil, err
		}
...
func newGitLabMRListRequest(ctx context.Context, apiBase, project string, page int) (*http.Request, error) {
