...
`), 0o644)
	...
	err := Run(Options{
		...
		OutPath:   out,
		ListPRs: func(ctx context.Context, cfg config.RepoConfig, token string) ([]openPR, error) {
...
func TestIsMajordomoInternalBranch(t *testing.T) {
	cases := []struct {
		...
		{"majordomo-inference-cache/demo", "bot/cache", true},
		{"main", "majordomo-inference-cache/demo", true},
...
func TestListGitHubPRsPaginated(t *testing.T) {
	...
	defer cancel()
	prs, err := listGitHubPRs(ctx, config.RepoConfig{
		SCM:        "github",
...
func TestListGitLabMRs(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	prs, err := listGitLabMRs(ctx, config.RepoConfig{
		SCM: "gitlab",
