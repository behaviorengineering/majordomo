...
func publishBitbucket(ctx context.Context, opts Options, summary string) error {
	...
	putDesc := func(prMeta map[string]any, description string) error {
		...
		_, _, err := httpJSON(ctx, c, "PUT", prURL, opts.BitbucketToken, payload)
		return err
	}

	postComment := func(text string) error {
		_, _, err := httpJSON(ctx, c, "POST", prURL+"/comments", opts.BitbucketToken, map[string]any{"text": text})
		return err
	}
	...
	case ModeDescription:
		prMeta, _, err := httpJSON(ctx, c, "GET", prURL, opts.BitbucketToken, nil)
		if err != nil {
			return err
		}
