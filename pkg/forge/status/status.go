...
func Run(opts Options) error {
	...
	opts.fillFromEnv()
	ctx, cancel := context.WithTimeout(context.Background(), outbound.DefaultTimeout)
	defer cancel()
	switch strings.ToLower(opts.SCM) {
	case "github":
		return postGitHub(ctx, opts)
	case "bitbucket":
		return postBitbucket(ctx, opts)
	default:
...
func postBitbucket(ctx context.Context, opts Options) error {
	...
	return doJSON(ctx, opts.client(), "POST", url, opts.BBToken, payload)
}

func postGitHub(ctx context.Context, opts Options) error {
	...
	return doJSON(ctx, opts.client(), "POST", url, opts.GitHubToken, payload)
}

func doJSON(ctx context.Context, client *http.Client, method, url, token string, body any) error {
