...
type DigestStore struct {
	Dir string
	// Now stamps CreatedAt on Store* writes. Zero is an error (job entry must set Now).
	Now time.Time
...
func (s *DigestStore) createdAt() (string, error) {
	if s == nil || s.Now.IsZero() {
		return "", fmt.Errorf("digest store: Now clock is required")
	}
	return s.Now.UTC().Format(timestampFmt), nil
}
...
func (s *DigestStore) flushAfterStore() error {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		if err := s.Flush(); err != nil {
			...
			if attempt == maxAttempts {
				return fmt.Errorf("digest cache flush: %w", last)
			}
			time.Sleep(backoff)
			backoff *= 2
			continue
