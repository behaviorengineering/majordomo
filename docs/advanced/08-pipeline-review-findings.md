...
**Recommendation:**
Move proxy config to `.majordomo-config.groovy` or Jenkins global environment variables. Ensure defaults are provided so non-Westpac deployments don't fail when the proxy is unavailable.

**Resolution (2026-05-22):** Accepted. This pipeline targets Westpac infrastructure only. A clarifying comment was added noting the override path via Jenkins global env vars or config.
