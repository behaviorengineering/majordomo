...
**Advanced Usage:**
- [Authentication and Username Resolution](#-authentication-and-username-resolution) - How `REGISTRY_USER` is used, including `@` splitting fallback
...
- **package registry Docker repository** to push and cache the `copilot-cli` image
...
python .majordomo/scripts/setup-majordomo.py \
    ...
    --repo-url ssh://git@bitbucket.example.com/.../<your-app-repo>.git \
    --create-job
```
...
| Flag | Default | Notes |
|------|---------|-------|
| `--jenkins-url` | `https://jenkins.example.com` | Target a different Jenkins instance |
| `--username` | local part of `REGISTRY_USER` | e.g. `ci-user` from `ci-user@example.com` |
...
Connecting to Jenkins: https://jenkins.example.com ...

Checking registry reachability ...
  ✅ registry.pullDomain: <package-registry-pull-domain>
  ✅ registry.pushDomain: <package-registry-push-domain>
...
Verifying credentials in Jenkins ...
  ...
  ✅ credentials.package-registryCredentialsId: <jenkins-package-registry-credential-id>
...
If `--username` is omitted, both scripts use `REGISTRY_USER` and take only the local part before `@`. For example, `ci-user@example.com` resolves to `ci-user`.
...
| **SCM** | Git | Git |
| **Repository URL** | `ssh://git@bitbucket.example.com/.../<your-app-repo>.git` | `ssh://git@bitbucket.example.com/example-project/majordomo.git` |
