...
variable instead of --api-token. The username defaults to the ARTIFACTORY_USER
...
_DEFAULT_JENKINS_URL = "https://jenkins.srv.westpac.com.au"
_HTTP_NOT_FOUND = 404
_HTTPS_PORT = 443
...
_CRED_EXPECTED_TYPES: dict[str, str] = {
    ...
    "credentials.artifactoryCredentialsId": "Username with password",
...
class CredentialsConfig:
    ...
    artifactory_credentials_id: str = ""
...
def parse_config(config_path: Path) -> PipelineConfig:
    ...
    return PipelineConfig(
        ...
        credentials=CredentialsConfig(
            artifactory_credentials_id=_extract_string(
                cred, "artifactoryCredentialsId"
            ),
...
def validate_config(cfg: PipelineConfig) -> ValidationResult:
    ...
    if not cfg.credentials.artifactory_credentials_id:
        result.errors.append("credentials.artifactoryCredentialsId is required")
...
def _run_credential_verification(
    ...
    checks: list[tuple[str, str]] = [
        ...
        ("credentials.artifactoryCredentialsId", creds.artifactory_credentials_id),
        (
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--username",
        metavar="USER",
        help="Jenkins username (default: ARTIFACTORY_USER env var)",
    )
    parser.add_argument(
        "--api-token",
        metavar="TOKEN",
...
def main() -> None:
    ...
    env_user = os.environ.get("ARTIFACTORY_USER", "").split("@")[0]
    username: str = str(args.username or "") or env_user

    missing: list[str] = []
    if not username:
        missing.append("--username or ARTIFACTORY_USER env var")
    if not api_token:
