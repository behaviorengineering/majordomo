...
Required env vars:
    SALARY_ID                Artifactory username (email or bare name — domain is stripped automatically)
    ARTIFACTORY_ACCESS_TOKEN  Artifactory token
...
_REGISTRY = "a01a0f-met-docker-snapshot-dependencies.artifactory.srv.westpac.com.au"
...
def _run_build(
    ...
    Args:
        ...
        salary_id_path: WSL Linux path to the salary_id secret file.
        jfrog_token_path: WSL Linux path to the jfrog_token secret file.
    ...
    try:
        ...
        cmd_str = (
            ...
            f"--secret id=salary_id,src={shlex.quote(salary_id_path)} "
            f"--secret id=jfrog_token,src={shlex.quote(jfrog_token_path)} "
...
def main() -> None:
    ...
    Reads SALARY_ID and ARTIFACTORY_ACCESS_TOKEN from environment variables.
    """
    dry_run = "--dry-run" in sys.argv
    verbose = "--verbose" in sys.argv or "-v" in sys.argv

    salary_id = os.environ.get("SALARY_ID", "").split("@")[0]
    jfrog_token = os.environ.get("ARTIFACTORY_ACCESS_TOKEN", "")

    if not dry_run and (not salary_id or not jfrog_token):
        missing = [v for v, val in (("SALARY_ID", salary_id), ("ARTIFACTORY_ACCESS_TOKEN", jfrog_token)) if not val]
        missing_str = ", ".join(missing)
        print(f"Error: required env vars not set: {missing_str}")
        print("Set SALARY_ID and ARTIFACTORY_ACCESS_TOKEN before running.")
        sys.exit(1)
    ...
    try:
        _wsl_podman_login(_REGISTRY, salary_id, jfrog_token)
    except subprocess.CalledProcessError as err:
        stderr = err.stderr.strip() if err.stderr else ""
        print("Error: failed to log in WSL Podman to Artifactory registry.")
        if stderr:
            print(stderr)
        sys.exit(1)
    ...
    salary_id_path = _write_wsl_secret("salary_id", salary_id)
    jfrog_token_path = _write_wsl_secret("jfrog_token", jfrog_token)
    try:
        for dockerfile in dockerfiles:
            tool = _tool_name(dockerfile)
            tag = _image_tag(tool)
            ...
            success, output = _run_build(
                dockerfile, workspace_root, salary_id_path, jfrog_token_path, tag
            )
            results[tool] = success
            _print_result(tool, success, output, verbose=verbose)
    finally:
        _delete_wsl_secret("salary_id")
        _delete_wsl_secret("jfrog_token")
