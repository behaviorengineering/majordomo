...
//   met-app-docker-creds        - Username/password for Artifactory Docker registry
//   met-app-artifactory-token   - SALARY_ID + JFROG_TOKEN for BuildKit secrets (apt/npm)

def DOCKER_CONFIG = [
    registry: [
        pullDomain:    'a01a0f-met-docker-snapshot-dependencies.artifactory.srv.westpac.com.au',
        pullUrl:       'https://a01a0f-met-docker-snapshot-dependencies.artifactory.srv.westpac.com.au',
        pushDomain:    'a01a0f-met-docker-snapshot-local.artifactory.srv.westpac.com.au',
        credentialsId: 'met-app-docker-creds',
    ],
    jenkinsAgent: [
    ...
    credentials: [
        artifactoryCredentialsId: 'met-app-artifactory-token',
    ],
]
            ...
            steps {
                script {
                    ...
                    withCredentials([
                        ...
                        usernamePassword(
                            credentialsId: DOCKER_CONFIG.credentials.artifactoryCredentialsId,
                            usernameVariable: 'SALARY_ID',
...
// Credentials (REGISTRY_USR, REGISTRY_PSW, SALARY_ID, JFROG_TOKEN) must be in scope
...
def ensureImage(deps, tags, Map dockerConfig, String slug, String dockerfile) {
    ...
    def sharedScript    = "${dockerfileDir}scripts/setup-artifactory-apt.sh"
    def userScript      = "${dockerfileDir}scripts/artifactory-user.sh"
