...
`majordomo context digest` seed may run an unattended Typology **cluster + refine** loop, then a **human-intervention** flagger for open architecture findings, and store a **proposal** under the served-repo context branch at `evidence/typology/` (`refined_snapshot.yaml`, `snapshot.yaml`, `journey.md`, `cluster_proposal.md`, post-refine `architecture_brief.md`, `human_intervention.md`, optional `pr_priority.md`, `graph.txt`, `manifest.yaml`). Discover drafts stay ephemeral under the analysis worktree (`tmp/typology/`) and are never committed to the context branch. Root context `architecture.md` is the teaching story; `architecture_brief.md` is Typology seed evidence. That proposal feeds the teaching story and, when findings remain, the context PR summary leads with priority human decisions written in a tutor voice for a cold reader, with consultant counsel (smell, alternatives, lean) rather than an inventory. Discovery journey notes argue the same way. Counsel attributes proposals to Majordomo/Typology digest on the context branch; it must not sound like a consented product-repo reorganization. When the proposal classifies packages as libraries, evidenced consumer-slice imports become `SliceBinding` entries that complete that classification; humans still decide whether library placement itself is right via merge or reject. It does **not** write or emit this repository's confirmed `.typology/typology.yaml`.

Humans promote a proposal into the confirmed catalog with the interactive Typology journey (load `typology-journey`), then `emit` / `validate` here. Digest proposes architecture grounding and flags normative decisions; operators own the confirmed map.
